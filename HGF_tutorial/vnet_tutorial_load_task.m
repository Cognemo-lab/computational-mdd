...
% to use the HGF and RW models and it consists of several parts:
%
...
u = load('vnet_task_example_inputs.txt');

%% PART 2: What is the task structure?
% Array that indicates the probability of the binary outcome = 1 for each
% experimental block
prob_struct = [0.8, 0.8, 0.20, 0.20];
n_trials    = 20;                  % Number of trials within a block
...
bopars = tapas_fitModel([], u(:,1),...           % experimental input
    'tapas_hgf_ar1_binary_config',...           % perceptual model function
    'tapas_bayes_optimal_binary_config',... % observational model function
    'tapas_quasinewton_optim_config');      % optimization function
tapas_hgf_ar1_binary_plotTraj(bopars);
...
sim = tapas_simModel(u(:,1),...
    'tapas_hgf_ar1_binary', bopars.p_prc.p,...
    'vnet_linear_binary', [log(10) 0 0 log(log(20))],...
    random_seed);
tapas_hgf_ar1_binary_plotTraj(sim);
hold on; plot(vol_struct, 'k:'); hold off; % Plot volatility structure 
...
est = tapas_fitModel(sim.y, sim.u,...
    'tapas_hgf_ar1_binary_config',...
    'vnet_linear_binary_config',...
...
fprintf('Parameters of ideal observer:\n omega: %.4f\n m: %.4f\n\n',...
    sim.p_prc.om(2), sim.p_prc.m(3))
fprintf('Recovered parameters:\n omega: %.4f\n m: %.4f\n\n',...
    est.p_prc.om(2), est.p_prc.m(3))
...
tapas_hgf_ar1_binary_plotTraj(est);
hold on; plot(vol_struct, 'k:'); hold off; % Plot volatility structure 
...
lr_hgf_high_lvl = est.traj.muhat(:,1).*(1-est.traj.muhat(:,1)).*est.traj.sahat(:,3).*est.traj.w(:,2);
