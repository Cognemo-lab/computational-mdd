...
% experimental block
prob_struct = [0.8, 0.8, 0.8, 0.2, 0.2, 0.2, 0.58, 0.58, 0.7, 0.7, 0.2, 0.2, 0.8, 0.8];
n_trials    = 10;                  % Number of trials within a block
...
fprintf('Parameters of ideal observer:\n omega: %.4f\n theta: %.4f\n\n',...
    sim.p_prc.ka(2), sim.p_prc.om(3))
fprintf('Recovered parameters:\n omega: %.4f\n theta: %.4f\n\n',...
    est.p_prc.ka(2), est.p_prc.om(3))
...
%% PART 5: Compare Precision-Weighted Prediction Errors
% Now let's compare the learning rates for the two models. To do this, we
% first need to compute them.
epsilon_low = est.traj.da(:,1).*est.traj.sa(:,2);
epsilon_high = est.traj.sa(:,3).*est.traj.da(:,2);
...
xlabel('Trials');
ylabel('PE Magnitudes');
legend('HGF Low PE', 'HGF High PE','Probability',...
    'Position', [0.78 0.6 0.1 0.2]);
