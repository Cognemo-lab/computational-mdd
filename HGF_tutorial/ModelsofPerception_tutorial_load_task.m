...
hold on;
plot(vol_struct, 'k');
plot(u(:,1),'.', 'Color', [0 0.6 0], 'MarkerSize', 12);
hold on; plot(u(:,2),'r');
hold off;
xlim([0, last_trial]);
ylim([-.05, 1.05]);
...
bopars = tapas_fitModel([], u,...           % experimental input
    'tapas_ehgf_binary_config',...           % perceptual model function
...
sim = tapas_simModel(u,...
    'tapas_ehgf_binary', bopars.p_prc.p,...
    'tapas_condhalluc_obs2', [5 1],...
    random_seed);
...
fprintf('Parameters of ideal observer:\n omega: %.4f\n nu: %.4f\n\n',...
    sim.p_prc.om(2), sim.p_prc.om(3))
fprintf('Recovered parameters:\n omega: %.4f\n nu: %.4f\n\n',...
    est.p_obs.nu, est.p_obs.nu)
